...
pub fn create_presale(
    ...
    // presale.presale_sol_account = ctx.accounts.presale_sol_account.key();
...
pub fn start_presale(ctx: Context<ManagePresale>) -> Result<()> {
    ...
    // Ensure presale is approved
    require!(
        presale.status == STATUS_APPROVED,
        IdoError::InvalidPresaleStatus
    );
    
    // Ensure start time has been reached
    require!(
        current_time >= presale.start_time,
        IdoError::InvalidTimeSetup
    );
    
    // Ensure presale hasn't ended
    require!(
        current_time < presale.end_time,
        IdoError::PresaleEnded
    );
    
    // Update status to live
    presale.status = STATUS_LIVE;
    
    msg!("Presale started successfully");
...
pub fn end_presale(ctx: Context<ManagePresale>) -> Result<()> {
    ...
    // Update global state
    let global_state = &mut ctx.accounts.global_state;
    global_state.active_presales = global_state.active_presales.saturating_sub(1);
...
pub fn buy_tokens(
    ...
    if amount > 0 && can_purchase_from_tier(3, user_tier, presale) {
        ...
        if tier3_purchase > 0 {presale.tier3_sold = presale.tier3_sold.checked_add(tier3_purchase).unwrap();
        }
    }
...
pub fn withdraw_funds(ctx: Context<WithdrawFunds>) -> Result<()> {
    ...
    // Calculate fees (5% to treasury)
    let total_amount = presale.sol_raised;
    let fee_amount = total_amount
        .checked_mul(5)
        .unwrap()
        .checked_div(100)
        .unwrap();
    let creator_amount = total_amount.checked_sub(fee_amount).unwrap();
...
pub fn list_token(
    ...
    require!(
        !presale.is_listed,
        IdoError::TokenAlreadyListed
    );
    
    // Calculate the amount of SOL to add to liquidity (80% of SOL raised)
    let sol_to_liquidity = presale.sol_raised
        .checked_mul(80)
        .unwrap()
        .checked_div(100)
        .unwrap();
    ...
    let tokens_to_liquidity = presale.tokens_sold
        .checked_mul(20)
        .unwrap()
        .checked_div(100)
        .unwrap();
...
pub struct WithdrawFunds<'info> {
    #[account(
        mut,
        constraint = creator.key() == presale.creator @ IdoError::Unauthorized
    )]
    pub creator: Signer<'info>,
    ...
    #[account(
        mut,
        constraint = treasury_wallet.key() == global_state.treasury_wallet
    )]
